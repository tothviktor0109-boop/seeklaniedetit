...
export default async function handler(req, res) {
    // Karakterkódolás beállítása az albán és magyar karakterekhez
    ...
    try {
        ...
        if (path === '/api/kassza') {
            ...
            if (method === 'POST') {
                ...
                // Fix: Adatok tömbbe zárása az insert-hez
                const { error } = await supabase.from('kassza_log').insert([{ tipus, osszeg, operator, bizonyitek }]);
        ...
        if (path === '/api/tagok') {
            if (method === 'GET') {
                ...
                // Rendezés prioritás szerint (növekvő)
                const rendezett = tagok.map(t => ({
                    ...t,
                    prioritas: rangok.find(r => r.rang === t.rang)?.prioritas || 999
                })).sort((a, b) => a.prioritas - b.prioritas);
                return res.json(rendezett);
            }
            if (method === 'POST') {
                if (!user?.tag_kezel && user?.rang !== 'DEV') return res.status(403).json({ error: 'Nincs jogod!' });
        ...
        if (path.startsWith('/api/tagok/') && method === 'PUT') {
            ...
            const { nev } = req.body;
            // Felhasználónév módosítás korlátozása (csak Prio 1-2 vagy DEV)
            if (nev && user.prio > 2 && user.rang !== 'DEV') {
                return res.status(403).json({ error: 'Csak 1-es vagy 2-es prioritással módosítható a felhasználónév!' });
            }
        ...
        if (path.startsWith('/api/tagok/') && method === 'DELETE') {
            await supabase.from('tagok').delete().eq('id', path.split('/').pop());
            return res.json({ success: true });
        }
        ...
        if (path.startsWith('/api/profil/')) {
            const id = path.split('/').pop();
            if (method === 'GET') {
                const { data } = await supabase.from('tagok').select('*').eq('id', id).single();
        ...
        if (path === '/api/jogosultsagok') {
            if (method === 'GET') {
                const { data } = await supabase.from('jogosultsagok').select('*').order('prioritas');
                return res.json(data);
            }
            if (method === 'POST') {
                if (!user?.jog_kezel && user?.rang !== 'DEV') return res.status(403).json({ error: 'Nincs jogod!' });
                await supabase.from('jogosultsagok').insert([req.body]);
                return res.json({ success: true });
            }
        }
        ...
        if (path.startsWith('/api/jogosultsagok/')) {
            ...
            if (method === 'PUT') {
                await supabase.from('jogosultsagok').update({ [req.body.mezo]: req.body.ertek }).eq('rang', r);
                return res.json({ success: true });
            }
            if (method === 'DELETE') {
                if (r === 'DEV') return res.status(400).json({ error: 'A DEV rang fix, nem törölhető!' });
                await supabase.from('jogosultsagok').delete().eq('rang', r);
                return res.json({ success: true });
            }
        }
        ...
        if (path === '/api/hirek') {
            ...
            if (method === 'POST') {
                await supabase.from('hirek').insert([{ ...req.body, iro: user.nev }]);
                return res.json({ success: true });
            }
        }

        if (path.startsWith('/api/hirek/') && method === 'DELETE') {
            await supabase.from('hirek').delete().eq('id', path.split('/').pop());
            return res.json({ success: true });
        }

        // --- JELSZÓCSERE ---
        if (path === '/api/jelszocsere' && method === 'POST') {
            await supabase.from('tagok').update({ jelszo: req.body.ujJelszo, elso_belepes: false }).eq('id', req.body.userId);
            return res.json({ success: true });
        }
